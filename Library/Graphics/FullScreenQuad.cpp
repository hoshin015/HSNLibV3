...
{
	CreateVsFromCso("Data/Shader/FullScreenQuad_VS.cso", embeddedVertexShader.ReleaseAndGetAddressOf(), nullptr,
	                nullptr, 0);
	CreatePsFromCso("Data/Shader/FullScreenQuad_PS.cso", embeddedPixelShader.ReleaseAndGetAddressOf());
}

void FullScreenQuad::blit(ID3D11ShaderResourceView** shaderResourceView, uint32_t startSlot, uint32_t numViews,
                          ID3D11PixelShader*         replacedPixelShader, ID3D11VertexShader* replacedVertexShader)
{
	...
	replacedVertexShader
		? dc->VSSetShader(replacedVertexShader, 0, 0)
...
void FullScreenQuad::blit(ID3D11ShaderResourceView** shaderResourceView, ID3D11ShaderResourceView** bloomFilterViews,
                          ...
                          ID3D11VertexShader* replacedVertexShader)
{
	Graphics*            gfx = &Graphics::Instance();
	ID3D11DeviceContext* dc  = gfx->GetDeviceContext();
