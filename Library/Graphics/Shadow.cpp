...
Shadow::Shadow(uint32_t width, uint32_t height)
{
	...
	for (int i = 0; i < SHADOWMAP_COUNT; i++)
	{
		...
		hr = device->CreateDepthStencilView(depthStencilBuffer.Get(), &depthStencilViewDesc,
		                                    depthStencilViews[i].GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), hrTrace(hr));
	...
	CreateVsFromCso("Data/Shader/ShadowMapCasterVS.cso", shadowVertexShader.ReleaseAndGetAddressOf(),
	                shadowInputLayout.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	CreateVsFromCso("Data/Shader/ShadowMapCasterStaticVS.cso", shadowStaticVertexShader.ReleaseAndGetAddressOf(),
	                shadowInputLayout.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
...
void Shadow::Activate(int index, CameraBase* camera)
{
	Graphics*            gfx = &Graphics::Instance();
	ID3D11DeviceContext* dc  = gfx->GetDeviceContext();
	...
	DirectX::XMVECTOR FRONT  = camera->GetFrontVec();
	DirectX::XMVECTOR RIGHT  = camera->GetRightVec();
	DirectX::XMVECTOR UP     = camera->GetUpVec();
	DirectX::XMVECTOR EYEPOS = camera->GetPosition();


	DirectX::XMVECTOR cameraFront = DirectX::XMVector3Normalize(FRONT);
	DirectX::XMVECTOR cameraRight = DirectX::XMVector3Normalize(RIGHT);
	DirectX::XMVECTOR cameraUp    = DirectX::XMVector3Normalize(UP);
	DirectX::XMVECTOR cameraPos   = EYEPOS;
...
#else
		// エリア近平面の中心から上面までの距離を求める
		float nearY = tanf(camera->GetFov() / 2.0f) * nearDepth;
		// エリア近平面の中心から右面までの距離を求める
		float nearX = nearY * camera->GetAspect();
		// エリア遠平面の中心から上面までの距離を求める
		float farY = tanf(camera->GetFov() / 2.0f) * farDepth;
		// エリア遠平面の中心から右面までの距離を求める
