...
float4 main(VS_OUT pin) : SV_TARGET
{
    ...
    float dither = InterleavedGradientNoise(pin.position.xy, 0);

    float f = 1.0 / float(samplingCount);
    for (int i = 0; i < samplingCount; i++)
    {
        float t = (i + dither) * f;
        c += textureMaps.Sample(samplerStates[_pointSampler], direction * lerp(1, 1 - blurPower, t) + blurCenter).rgb;
    }

    float4 color = float4(c*f, 1.0);
